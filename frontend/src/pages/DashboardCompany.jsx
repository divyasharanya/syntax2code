...
const DashboardCompany = () => {
  ...
  const companyTasks = tasks.filter((t) => String(t.companyId) === String(user.id));
  const companyTaskIds = companyTasks.map((t) => t.id);

  // Get submissions corresponding to this company's tasks
  const companySubmissions = submissions.filter((sub) => companyTaskIds.map(String).includes(String(sub.taskId)));
  ...
  if (selectedSubId) {
    const sub = submissions.find((s) => String(s.id) === String(selectedSubId));
    if (!sub) {
      setSelectedSubId(null);
      return null;
    }
